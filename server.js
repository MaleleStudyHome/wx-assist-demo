...
app.get('/api/agent/status', (req, res) => {
  ok(res, { bound: false, ready: true, tools: ['summarize', 'search', 'notify', 'configure'] })
})
app.post('/api/agent/chat', (req, res) => {
  ...
  const steps = [
    { type: 'step', status: 'running', step: '正在读取消息...' },
    { type: 'step', status: 'done', step: '消息读取完成', result: '186 条' },
    { type: 'step', status: 'running', step: 'AI 提炼摘要...' },
    { type: 'token', content: '摘要内容生成中...' },
    { type: 'done', summary: 'Mock summary', notifications: [] },
  ]

  let idx = 0
  const timer = setInterval(() => {
    if (idx < steps.length) {
      res.write(`event: ${steps[idx].type}\ndata: ${JSON.stringify(steps[idx])}\n\n`)
      idx++
    } else {
      clearInterval(timer)
      res.end()
...
app.post('/api/agent/rag-search', (req, res) => {
  ok(res, { results: [
    { chat_id: '123@chatroom', group_name: '项目核心群', sender: '张三', time: '2026-03-12 14:32', content: 'Redis 做缓存确实快，但咱们写多读少，命中率太低', relevance: 0.92 },
    { chat_id: '123@chatroom', group_name: '项目核心群', sender: '李芳', time: '2026-03-12 15:10', content: '换成本地缓存？内存映射文件就行，部署也简单', relevance: 0.88 },
  ]})
})
